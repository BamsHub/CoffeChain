...
export default function ContactPage() {
    ...
    useEffect(() => {
        fetch('/api/auth/me').then(r => r.json()).then(d => { if(d.success && d.user) setUserRole(d.user.role); }).catch(()=>{});
    }, []);
...
const [name, setName] = useState('');
 ...
 <nav style={styles.nav}>
                    {(userRole === 'admin' || userRole === 'petani') ? (
                        <a href="/dashboard" style={styles.navLinkPrimary}>Dashboard</a>
                    ) : (
                        <>
